...
4. **単一ファイルに全機能集約**: スコア計算やDOM操作が1ファイルに集中し、保守性が低い
5. **グローバル変数の多用**: 8個のグローバル変数が関数間で共有され、予期しない副作用のリスクがある
6. **エラー処理の不備**: 多くの箇所でエラーハンドリングが不十分
7. **非同期処理の非効率性**: `reDraw()`内で逐次待機処理が行われている
